...
if __name__ == '__main__':
    ...
    # Apply undistortion to the selected images and display them
    for i, img in enumerate(img_select):
        undistorted_img = cv.undistort(img, K, dist_coeff)

        cv.imwrite(f'Original_Image_{i+1}.png', img)
        cv.imwrite(f'Undistorted_Image_{i+1}.png', undistorted_img)
